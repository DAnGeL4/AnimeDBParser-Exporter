...
class WebPageService:
    ...
    def _get_page_filename(self, type: WatchListTypes, page_filename: str) -> str:
        '''
    ...
    def is_exist_web_page_file(self, type: WatchListTypes, page_filename: str) -> bool:
    ...
    def load_web_page_file(self, type: WatchListTypes, page_filename: str) -> cfg.WebPage:
        '''
        '''
        web_page = None
        file_name = self._get_page_filename(type, page_filename)
    ...
    def save_web_page(self, type: WatchListTypes, page_filename: str, 
        ...
        if not os.path.exists(self.dir_path):
            self._logger.info(f"Directory \"{self.dir_name}\" not exists. Creating...")
            try:
    ...
    def load_json_data(self, module_name: str, json_dump_name: str) -> typ.Dict[str, typ.Any]:
        '''
    ...
    def prepare_json_data(self, json_data: typ.Dict[str, typ.Any],
                          type: WatchListTypes) -> typ.Dict[str, typ.Any]:
        '''
...
class WebPageParser(ac.WebPageParserAbstract):
    ...
    def _get_web_page(self, anime_key: str, anime_url: AnyHttpUrl) -> cfg.WebPage:
    ...
    def get_anime_info_json(self, anime_item: typ.Tuple[str, AnyHttpUrl],
        ...
        if not self._if_update_needed(anime_key, json_data): 
            return None
        ...
        json_data[self._type.value].update({
            anime_key: anime_info._asdict()
        })
    ...
    def get_anime_data_in_multithreads(self, all_anime_urls: typ.Dict[str, AnyHttpUrl], 
                                       json_data: AnimeByWatchList
            ...
            for anime_item in all_anime_urls.items():
                futures.append(
                    executor.submit(self.get_anime_info_json, 
                                    anime_item=anime_item, json_data=json_data)
                )
    ...
    def get_anime_data(self, web_page: WebPage, 
